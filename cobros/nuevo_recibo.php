...
$sel_facturas="SELECT codcliente, moneda FROM facturas WHERE codfactura=$codfactura";
$res_facturas=mysqli_query($GLOBALS["___mysqli_ston"], $sel_facturas);
$codcliente=mysqli_result($res_facturas, 0, "codcliente");
$moneda=mysqli_result($res_facturas, 0, "moneda");
...
$sel_borrar = "DELETE FROM recibosfacturatmp WHERE codrecibo>='1'";
$rs_borrar = mysqli_query($GLOBALS["___mysqli_ston"], $sel_borrar);

$sel_borrar = "DELETE FROM recibospagotmp WHERE codrecibo>='1'";
$rs_borrar = mysqli_query($GLOBALS["___mysqli_ston"], $sel_borrar);
					...
					<table class="fuente8" width="98%" cellspacing=0 cellpadding=2 border=0>
						<tr>
					      ...
					      <td><input name="codcliente" type="text" class="cajaPequena" id="aCliente" size="6" maxlength="5" onClick="limpiarcaja()" value="<?php echo $codcliente;?>" data-index="1">
						    ...
						    <td width="27%"><input name="nombre" type="text" class="cajaGrande" id="nombre" size="45" maxlength="45" value="" readonly></td>
						</tr>
						<tr>
				            <td width="5%">RUT</td>
...
?>						
								<label><?php echo $moneda2;?> -> <?php echo $moneda1;?>$&nbsp;</label>
						...
						<td>Moneda</td><td>
						 <select name="Amonedapago" id="Amonedapago" class="cajaPequena2" data-index="9" onchange="cambio();">
							<?php //$tipomon= array( 0=>"Seleccione uno", 1=>"Pesos", 2=>"U\$S");
							foreach ($tipomon as $key => $i ) {
								if($moneda==$key) {
									echo "<option value=$key selected>$i</option>";
								} else {
				...
				<table class="fuente8" cellspacing=0 cellpadding=3 border=0>
					...
					<div align="center">
						...
						<button class="boletin" onClick="event.preventDefault();cancelar('<?php echo $codrecibo;?>','<?php echo $codcliente;?>');;" onMouseOver="style.cursor=cursor"><i class="fa fa-ban" aria-hidden="true"></i>&nbsp;Salir</button>
