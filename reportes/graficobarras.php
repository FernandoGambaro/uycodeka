...
$dia_fin =date("Y-m-d", mktime( 0, 0, 0, $mes, $fin, $year ));
 ...
 $sql_total="SELECT horas.codusuario,horas.horas,horas.codcliente, clientes.nombre, clientes.empresa FROM
 `horas`, clientes WHERE horas.codcliente=clientes.codcliente AND horas.codusuario='$codusuarios' 
 AND horas.borrado=0 AND `fecha` BETWEEN '$dia_ini' AND '$dia_fin' GROUP BY codcliente ORDER BY `fecha` DESC";
...
$total_usuario='00:00:00';
 ...
 `horas` INNER JOIN  clientes on  horas.codcliente=clientes.codcliente WHERE horas.codcliente=$codcliente AND horas.codusuario='$codusuarios' 
 ...
 //echo $sql_proyecto."<br>";
   	...
   	$total_usuario=SumaHoras($row_proyecto['horas'].":00",$total_usuario);
   	//echo "<br>";
			$array = split(":", $total_usuario);
			$total_usuario1=($array[0]*60+$array[1])/60;
			if($total_usuario1>0) {
				//$datay[$i]=floor($total_usuario1);
				$datay[$t]=floatval($total_usuario1);
				$tot=$tot+$total_usuario1;
			} else {
				//$datay[$i]=0;
				$datay[$t]=0;
			}
...
$graph->Stroke();
	...
	echo "No hay datos de ese usuario para mostrar";
}
