...
$codfacturatmp=$codfactura=$_GET["codfactura"];
$accion=$_GET['accion'];
...
while ($contador < mysqli_num_rows($rs_lineas)) {
	...
	$sel_tmp="INSERT INTO factulineatmp (codfactura,numlinea,codfamilia,codigo,codservice,detalles,cantidad,moneda,precio,importe,dcto,dctopp,comision) 
				VALUES ('$codfactura','','$codfamilia','$codigo','$codservice','$detallestmp','$cantidad','$moneda','$precio','$importe','$dcto','$descuentopp','$comision')";	
				...
				<form id="formulario" name="formulario" method="post" action="guardar_factura.php">
					...
					?>
					  ...
					  ?>
						...
						<td>Moneda</td><td> 
						<select onchange="cambio();" name="Amoneda" id="Amoneda" class="cajaPequena2" data-index="4">
<?php
						$sel_resultado="SELECT * FROM monedas WHERE borrado=0 AND orden <3 ORDER BY orden ASC";
						   $res_resultado=mysqli_query($GLOBALS["___mysqli_ston"], $sel_resultado);
						   $contador=0;
			  ...
			  <input id="accion" name="accion" value="<?php echo $accion;?>" type="hidden">			  
			  </form>
			  ...
			  <div id="frmBusqueda">
				...
				<br style="line-height:5px">
					<div align="center">
						<button class="boletin" onClick="validar_cabecera();" onMouseOver="style.cursor=cursor"><i class="fa fa-minus" aria-hidden="true"></i>&nbsp;Eliminar</button>
