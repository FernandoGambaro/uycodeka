...
function Auth() {
  global $nombre, $password, $status_msg, $UserID, $UserTpo, $UserApe, $UserNom, $UserMail, $UserTra, $ask;
	...
	//return false;
	 ...
	 if ($posicion === false and $nombre!='Admin') {
	    ...
	    $encoded = $converter->encode($str );
		 $c_usuario = "SELECT usuario,contrasenia,codusuarios,tratamiento,nombre,apellido,ask, email FROM `usuarios` WHERE `usuario`='$nombre' AND `contrasenia`='$encoded' AND `estado`=0";
	    ...
	    $r_ok = @mysqli_fetch_array($r_usuario);
			    if($r_ok['usuario'] != $nombre && $r_ok['contrasenia'] != $encoded){
				     $status_msg .= $nombre." El usuario o la contraseña que ha ingresado no son correctas";
				     $nombre="";
				     $password="";
				     return false;
			    } elseif($r_ok['usuario'] == $nombre && $r_ok['contrasenia'] != $encoded){
				     $status_msg .= $nombre." Contraseña ingresada inválida";
				     $nombre="";
				     $password="";
				     return false;
					} else  {	
		...
		//}
	 	...
	 	$status_msg=$nombre." El usuario no existe o no esta habilitado";
	 }
}
}
...
if (password === '' || nombre === '') {
	...
	$('#msg').text('Complete los datos');

return false;
} else if (!(nombre).match(emailReg)) {
	$('#msg').text('Usuario invalido');
    ...
    <br>
        ...
        <div id="slide" aria-labelledby="ui-id-1" class="ui-tabs-panel ui-widget-content ui-corner-bottom" role="tabpanel" aria-expanded="true" aria-hidden="false">
            ...
            <div class="cont-right2">
                ...
                <div id="msg" class="errors"><?php echo $status_msg; ?></div>
		        ...
		        <form id="form_id" class="form-signin" name="login" method="POST" action="<?php echo $_SERVER['PHP_SELF'] ?>">
                        <div>
                                ...
                                <div class="input-group">
                                    ...
                                    <input id="nombre" name="nombre" class="input form-control" tabindex="1" placeholder="Usuario" accesskey="n" type="text" value="" size="25" autocomplete="off">
                                </div>
                        ...
                        <input  style="display:none" class="submit" name="OKsubmit" accesskey="l" value="Ingresar" tabindex="4" type="submit">
                    </form>
                                    ...
                                    <div class="cont-left">
                                        <div class="cont-block">
                                            <div class="cont-funcionarios ml5">
                                                <h3><?php echo $row["nombre"];?><br> Facturación WEB</h3>
