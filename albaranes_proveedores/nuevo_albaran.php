...
$codalbarantmp=((is_null($___mysqli_res = mysqli_insert_id($GLOBALS["___mysqli_ston"]))) ? false : $___mysqli_res);

$del_albaranptmp="DELETE FROM `albalineaptmp` WHERE `albalineaptmp`.`codalbaran` = '".$codalbarantmp."'";
$rs_albaran=mysqli_query($GLOBALS["___mysqli_ston"], $del_albaranptmp);
...
function pon_prefijo_Fb (codfamilia,pref,nombre,descripcion_corta,precio,codarticulo,moneda) {
	...
	actualizar_importe();
		...
		function abreVentana(){
			...
			if (bi==0) {
		   	...
		   	href: "ver_proveedores.php", open:true,
				...
				alert ("Ha comenzado la albaran. No puede cambiar de proveedor");
			}
		}
		...
		function actualizar_importe()
			{
				/*Si la albaran es en peso y el articulo esta en dolares aplico el tipo de cambio*/
				var tipocambioalbaran=document.formulario.amoneda.options[document.formulario.amoneda.selectedIndex].value;
				var tipocambioarcticulo=document.getElementById("moneda").value;
				if (tipocambioalbaran==1 && tipocambioarcticulo == 2){
					var precio=document.getElementById("precio").value * parseFloat(document.getElementById("tipocambio").value);
				}
				if (tipocambioalbaran==2 && tipocambioarcticulo == 1){
					var precio=document.getElementById("precio").value / parseFloat(document.getElementById("tipocambio").value);
				}
				if ((tipocambioalbaran==1 && tipocambioarcticulo == 1) || (tipocambioalbaran==2 && tipocambioarcticulo == 2)){
	...
	<body onload="inicio();">
		<div id="pagina">
			<div id="zonaContenido">
				<div align="center">
				<div id="tituloForm" class="header">INSERTAR ALBARAN / ORDEN DE PEDIDO</div>
				<div id="frmBusqueda">
				<form id="formulario" name="formulario" method="post" action="guardar_albaran.php">
					<table class="fuente8" width="98%" cellspacing="0" cellpadding="3" border="0">
						<tr>
						  ...
						  <td>Cod.&nbsp;albaran</td>
						  <td><input name="calbaran" type="text" class="cajaMedia" id="calbaran" size="20" maxlength="20" value="<?php echo $codalbarantmp;?>"></input></td>
						<?php $hoy=date("d/m/Y"); ?>
							<td>Fecha</td>
