...
*/
/*
$arriba = imagecreatefrompng("../img/content.png");
$width   =    imagesx ( $arriba );
$height  =    imagesy ( $arriba );
$cent_width   =    (250-$width)/2;
$cent_height  =   (135-$height)/2;
*/
...
$vence = implota(date ( 'Y-m-j' , $vence ));	
   ...
   $frace='';
	...
	if($s!='') {
	$cantlineas=0;
	$frace=wordwrap($s, $wmax, "\n", true );
	$obs= explode("\n", $frace);
	//var_dump($obs);
		if(is_array($obs) ) {
			while( $filasadenda>0 or $cantlineas>=3) {
			$pdf->Cell(84); 
			$pdf->Cell(87,4,$obs[$cantlineas],0,0,'L',0);	
			$pdf->Ln(4);	
			$cantlineas++;
			$filasadenda--;
			}
...
//$reporte=1;
	...
	$pdf->Output($file, 'F');
		if (file_exists($file) and $printer!='') {
		 ...
		 //$comando = escapeshellcmd('lp  -d '.$printer.' -n 1 -o media=Custom.210x297mm -o fit-to-page '.$file);
		...
		<script type="text/javascript" >
		      ...
		      window.opener.callGpsDiag(xx,numDoc,codfactura);
