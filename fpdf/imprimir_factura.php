...
$cent_width   =    (150-$width)/2;
$cent_height  =   (70-$height)/2;
...
/*1er línea*/
   ...
   $pdf->SetTextColor(255,255,255);
	...
	$pdf->Cell(53,6,'RUT EMISOR',1,0,'C',1);	
	$pdf->Cell(1,6,'',1,0,'C',0);	
	$pdf->Cell(55,6,'TIPO DE DOCUMENTO',1,0,'C',1);	
/*2da línea*/
   ...
   $pdf->SetTextColor(0,0,0);
	...
	$pdf->Cell(53,6,$nif,'L',0,'C',0);	
	$pdf->Cell(1,6,'',0,0,'C',0);	
	$pdf->Cell(55,6,$tipof,'R',0,'C',0);	
/*3er línea*/
	$pdf->Ln(6);
	 $pdf->Cell(90); /*  ubicación inicial de la celdas */
   $pdf->SetFont('Arial','B',11);
   $pdf->SetTextColor(255,255,255);
	...
	$pdf->Cell(1,6,'',0,0,'C',0);	
	$pdf->Cell(20,6,'NÚMERO',1,0,'C',1);	
	$pdf->Cell(1,6,'',0,0,'C',0);	
	$pdf->Cell(35,6,'FORMA DE PAGO',1,0,'C',1);	
	$pdf->Cell(1,6,'',0,0,'C',0);	
	$pdf->Cell(31,6,'VENCIMIENTO',1,0,'C',1);	
/*4ta línea*/
...
/*5ta línea*/
   ...
   $pdf->SetTextColor(255,255,255);
	$pdf->SetFillColor(0,0,0);
	$pdf->Cell(109,6,'RUT COMPRADOR: '.$rutcomprador ,1,0,'C',1);	
/*6ta línea*/
	$pdf->Ln(6);
   $pdf->SetFont('Arial','B',11);
   $pdf->SetTextColor(0,0,0);
...
if($lafila["empresa"]!='') {
	...
	$pdf->Cell(109,6,$nombre ,'LR',0,'C',0);	
/*7ma línea*/
	$pdf->Ln(6);
...
if($logofactura==1) {
	...
	$pdf->Cell(109,6,$lafila["direccion"],'LR',0,'C',0);
/*8va línea*/
	$pdf->Ln(6);
if($logofactura==1) {
	...
	$pdf->Cell(109,6,$provincias." - ".$pais,'LR',0,'C',0);
...
if($logofactura==1) {
   ...
   $pdf->SetTextColor(255,255,255);
	$pdf->SetFillColor(0,0,0);
	$pdf->Cell(53,6,'FECHA DE DOCUMENTO',0,0,'C',1);	
	$pdf->Cell(1,6,'',0,0,'C',0);	
	$pdf->Cell(55,6,'MONEDA',0,0,'C',1);	
/*10ma línea*/
   ...
   $pdf->SetTextColor(0,0,0);
	$pdf->SetFillColor(255,255,255);
	$pdf->Cell(53,6,$fecha,'LB',0,'C',0);	
	$pdf->Cell(1,6,'','B',0,'C',0);	
	$pdf->Cell(55,6,mysqli_result($res_resultado, ($lafila["moneda"]-1), 'simbolo')." - ".mysqli_result($res_resultado, ($lafila["moneda"]-1), 'descripcion'),'RB',0,'C',0);	
...
$importe32='';
	...
	$pdf->Cell(27,4,mysqli_result($rs_articulos, 0, "referencia"),0,0,'L',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(86,4,$acotado,0,0,'L',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(8,4,'',0,0,'C',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(21,4,$precio32,0,0,'R',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(21,4,mysqli_result($resultado32, $lineas, "cantidad"),0,0,'R',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(21,4,$importe32,0,0,'R',0);	
   ...
   $pdf->SetTextColor(0,0,0);
	...
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(30,4,'Subtotal gravado:',0,0,'R',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(18,4,$importe4,0,0,'R',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(34,4,'Total IVA: '.$ivatxt,0,0,'R',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
if($lugarmon==1) {	
	...
	$pdf->Cell(50,4,'',0,0,'C',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(30,4,'Subtotal no gravado:',0,0,'R',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(18,4,'0.00',0,0,'R',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(34,4,'TOTAL A PAGAR:',0,0,'R',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
if($lugarmon==1) {	
	$pdf->Cell(9,4,mysqli_result($res_resultado, ($lafila["moneda"]-1), 'simbolo')." ",0,0,'R',0);	
...
$vence = implota(date ( 'Y-m-j' , $vence ));	
   ...
   $pdf->SetFont('Arial','',9);
	$pdf->Cell(12); /*  ubicación inicial de la celdas */		
	$pdf->Cell(188,4,'Forma de pago: '.$forma,'LR',0,'L',0);	
	$filasadenda--;
	$pdf->Ln(4);
   ...
   $wmax=188; 
	...
	$obs= explode("\n", wordwrap($s, $wmax));
		if(is_array($obs) ) {
			...
			$pdf->Cell(12); /*  ubicación inicial de la celdas */		
			$pdf->Cell(188,4,@$obs[$cantlineas],'LR',0,'L',0);	
			$filasadenda--;
			$pdf->Ln(4);	
			$cantlineas++;
			}
...
/*Datos para el códifo QR*/
  ...
  file_put_contents("../tmp/".$filename, $qr);
   ...
   $pdf->SetTextColor(0,0,0);
	...
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(85,4,'Rec. Nro.',0,0,'L',0);	
	$pdf->Ln(5);
	$pdf->Cell(15); /*  ubicación inicial de la celdas */		
	$pdf->Cell(37,4,'',0,0,'C',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(85,4,'Puede verificar comprobante en www.dgi.gub.uy',0,0,'L',0);	
	$pdf->Ln(5);
	$pdf->Cell(15); /*  ubicación inicial de la celdas */		
	$pdf->Cell(37,4,'',0,0,'C',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(85,4,'I.V.A. al día',0,0,'L',0);	
	$pdf->Ln(5);
	$pdf->Cell(15); /*  ubicación inicial de la celdas */		
	$pdf->Cell(37,4,'',0,0,'C',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(85,4,'Nro. de CAE',0,0,'L',0);	
	$pdf->Ln(5);
	$pdf->Cell(15); /*  ubicación inicial de la celdas */		
	$pdf->Cell(37,4,'',0,0,'C',0);	
	$pdf->Cell(1,4,'',0,0,'C',0);	
	$pdf->Cell(85,4,'Rango de CAE:',0,0,'L',0);	
   ...
   $pdf->SetFont('Arial','',9);
	...
	$pdf->Cell(50,4,'Código de seguridad:',0,0,'C',0);	
...
//		$output = shell_exec('lp  -d'.$printer.' -n 2 -omedia=210x145mm -o fit-to-page '.$file); /*Envío la impresión por sistema*/
