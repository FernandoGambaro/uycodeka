        ...
        var index = parseFloat($this.attr('data-index'));
	        if (index==1) {
	        	var codigo=document.getElementById("nombre").value;
	        		if (codigo=='') {
						showWarningToast("Debe introducir antes el proveedor");
	        			$('[data-index="1"]').focus();
	        			return false;
		        		}
		     }
	        if (index==2) {
	        	var cfactura=document.getElementById("cfactura").value;
	        		if (cfactura=='') {
						showWarningToast("Debe introducir número factura");
	        			$('[data-index="2"]').focus();
	        			return false;
		        		}
		     }		  
		     if (index==4) {
		     		var tipoa=document.formulario.tipo.options[document.formulario.tipo.selectedIndex].value;
					if (tipoa==0) {showWarningToast("  - Seleccione tipo documento<br>");
					$("#tipo").focus();	
					return false;
					}
		     }		        
		     if (index==5) {
		     		var tipoamoneda=document.formulario.amoneda.options[document.formulario.amoneda.selectedIndex].value;
					if (tipoamoneda==0) {showWarningToast("  - Seleccione moneda<br>");
					$("#amoneda").focus();	
					return false;
					}
		     }
		      	...
		      	if (codigo=='') {
						...
						if (codprov=="") {
							...
							return false;
			     			...
			     			var cfactura=document.getElementById("cfactura").value;
							if (cfactura=='') {
							showWarningToast("Debe introducir numero factura");
...
function round(value, exp) {
  ...
  return +(value[0] + 'e' + (value[1] ? (+value[1] - exp) : -exp));
		...
		function abreVentana(){
			...
			if (bi==0) {
		   	...
		   	href: "ver_proveedores.php", open:true,
				iframe:true, width:"550", height:"480",
					onCleanup:function() {
						$('#cfactura').focus();
						}
				});			
		...
		function validar() 	{
				...
				if (document.getElementById("codbarras").value=="") mensaje="  - Falta Artículo<br>";
	...
	<body onload="inicio();">
		<div id="pagina">
			<div id="zonaContenido">
				<div align="center">
				<div id="tituloForm" class="header">NUEVA FACTURA COMPRA</div>
				<div id="frmBusqueda">
				<form id="formulario" name="formulario" method="post" action="guardar_factura.php">
					<table class="fuente8" width="98%" cellspacing="0" cellpadding="3" border="0">
						<tr>
						  ...
						  <td>Cod.&nbsp;Factura</td>
						  <td><input name="cfactura" type="text" class="cajaMedia" id="cfactura" size="20" maxlength="20" data-index="2"></input></td>
						<?php $hoy=date("d/m/Y"); ?>
							<td>Fecha</td>
						    <td width="14%"><input name="fecha" type="text" class="cajaPequena" id="fecha" size="10" maxlength="10" value="<?php echo $hoy?>"
						     onblur="busco_tipocambio();" onchange="busco_tipocambio();" data-index="3">
...
<?php
						...
						<tr>
							...
							<td colspan="4">Tipo&nbsp;cambio
								...
								<input name="tipocambio" type="text" class="cajaPequena2" id="tipocambio" size="5" maxlength="5" onchange="cambio();"
								onblur="cambio();" data-index="7"></span>
							</td>
						</tr>
					</table>										
			  </div>
			  <input id="codfacturatmp" name="codfacturatmp" value="<?php echo $codfacturatmp?>" type="hidden">
			  <input id="baseimpuestos2" name="baseimpuestos" value="" type="hidden">
			  ...
			  <div id="frmBusqueda">
				...
				<table class="fuente8" width="98%" cellspacing=0 cellpadding=2 border=0>
				  <tr>
					<td width="11%">Codigo barras/Referencia </td>
					<td>
					...
					<input type="text" size="26" maxlength="60" id="articulos" name="articulos" autocomplete="off" class="cajaMedia" onBlur="validarArticulo();" data-index="8"/>
					 ...
					 <img id="botonBusqueda" src="../img/calculadora.jpg" border="0" align="absmiddle" onClick="validarArticulo();" onMouseOver="style.cursor=cursor" title="Validar codigo de barras" style="vertical-align: middle; margin-top: -1px;">
					...
					<td>Moneda</td>					 
					 ...
					 <input name="codarticulo" id="codarticulo" type="hidden" >
					 </td>
				  </tr>
				  <tr>
					...
					<td width="11%"><input name="precio" type="text" class="cajaPequena2" id="precio" size="10" maxlength="10" onChange="actualizar_importe()" data-index="11"></td>
					<td width="5%">Cantidad</td>
					<td width="5%"><input name="cantidad" type="text" class="cajaMinima" id="cantidad" size="10" maxlength="10" value="1" onChange="actualizar_importe()" data-index="12"></td>
					<td width="4%">Dcto.</td>
					<td width="9%"><input name="descuento" type="text" class="cajaMinima" id="descuento" size="10" maxlength="10" onChange="actualizar_importe()" data-index="13"> %</td>
					...
					<td>
						<button class="boletin" onClick="event.preventDefault();validar();" onMouseOver="style.cursor=cursor"><i class="fa fa-plus" aria-hidden="true"></i>&nbsp;Agregar</button>
						</td>
					<td align="right">
					<button class="boletin" onClick="limpiar();" onMouseOver="style.cursor=cursor"><i class="fa fa-ban" aria-hidden="true"></i>&nbsp;Limpiar</button></td>
