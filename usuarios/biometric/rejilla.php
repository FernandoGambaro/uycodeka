...
$where="1=1";
if ($equipo <> "") { $where.=" AND equipo='$equipo'"; }
if ($codubicacion <> "0") { $where.=" AND codubicacion like '%".$codubicacion."%'"; }

$where.=" ORDER BY codbiometric ASC";
$query_busqueda="SELECT count(*) as filas FROM biometric WHERE borrado=0 AND ".$where;
...
$('.trigger').click(function(e){
      ...
      idstyle=this.id;
		...
		function ver_equipo(codbiometric) {
			var url="ver_equipo.php?codbiometric=" + codbiometric;
			window.parent.OpenNote(url);
		}
		
		function modificar_equipo(codbiometric) {
			var url="modificar_equipo.php?codbiometric=" + codbiometric;
			window.parent.OpenNote(url);
		}
		
		function eliminar_equipo(codboimetric) {
			var url="eliminar_equipo.php?codbiometric=" + codbiometric;
			window.parent.OpenNote(url);
		}
		...
		function act_db(codbiometric) {
			showModal(1);
		jQuery.ajax({
		 type: "POST",
		 url: "act_db_datosusuarios.php",
		 data: {codbiometric:codbiometric },
		 async: true,
		 cache: false,
		 success: function(data) { 
	...
	<body onload="inicio();">	
		<div id="pagina">
			...
			<div align="center">
			<div class="header">Listado de equipos biometricos </div>
			...
			<input type="hidden" name="numfilas" id="numfilas" value="<?php echo $filas;?>">
				...
				if ($iniciopagina>$filas) { $iniciopagina=0; }
					if ($filas > 0) { ?>
						<?php
						  $sel_resultado="SELECT * FROM biometric WHERE  ".$where;
						   $sel_resultado=$sel_resultado." limit ".$iniciopagina.",".$paginacion;
						   $res_resultado=mysqli_query($GLOBALS["___mysqli_ston"], $sel_resultado);
						   $contador=0;
						   while ($contador < mysqli_num_rows($res_resultado)) { 
								 if ($contador % 2) { $fondolinea="itemParTabla"; } else { $fondolinea="itemImparTabla"; }?>
						<tr id="<?php echo mysqli_result($res_resultado, $contador, "codbiometric");?>"	 class="<?php echo $fondolinea?> trigger">
							<td width="18px"><div align="center">
							<?php
							if(mysqli_result($res_resultado, $contador, "internal_id")==0) {
							?>
								<a href="#"><img id="botonBusqueda" src="../../img/comerce.png" width="16" height="16" border="0" onClick="act_db('<?php echo mysqli_result($res_resultado, $contador, "codbiometric")?>')" title="Actualizar DB del servidor"></a>
							<?php
							} else {
							?>
							<a href="#"><img id="botonBusqueda" src="../../img/zkteco16x16.png" width="16" height="16" border="0" onClick="act_device('<?php echo mysqli_result($res_resultado, $contador, "codbiometric")?>')" title="Agregar todos los usuarios al lector"></a></div></td>
							<?php
							}
							?>
							</div>
							</td>
							<td width="18px"><div align="center"><a href="#"><img id="botonBusqueda" src="../../img/aniadir.png" width="16" height="16" border="0" onClick="descargar_info('<?php echo mysqli_result($res_resultado, $contador, "codbiometric");?>')" title="Descargar registros del equipo"></a></div></td>
							
							<td><div align="left"><?php echo mysqli_result($res_resultado, $contador, "nombre");?>	</div></td>
							<td class="aDerecha"><div align="center"><?php
								$codubicacion=mysqli_result($res_resultado, $contador,  "codubicacion");
					  			$query_ubicacion="SELECT nombre FROM ubicaciones WHERE codubicacion='".$codubicacion."' ";
								$res_ubicacion=mysqli_query($GLOBALS["___mysqli_ston"], $query_ubicacion);
								echo mysqli_result($res_ubicacion,0, "nombre");
								?>							
							</div></td>
							<td class="aDerecha"><div align="left"><?php echo mysqli_result($res_resultado, $contador, "direccionip");?>	</div></td>
							<td class="aDerecha"><div align="center"><?php echo $estado[mysqli_result($res_resultado, $contador, "borrado")];?></div></td>

							<td width="18px"><div align="center"><a href="#"><img id="botonBusqueda" src="../../img/modificar.png" width="16" height="16" border="0" onClick="modificar_equipo(<?php echo mysqli_result($res_resultado, $contador, "codbiometric")?>)" title="Modificar"></a></div></td>
							<td width="18px"><div align="center"><a href="#"><img id="botonBusqueda" src="../../img/ver.png" width="16" height="16" border="0" onClick="ver_equipo(<?php echo mysqli_result($res_resultado, $contador, "codbiometric")?>)" title="Visualizar"></a></div></td>
							<td width="18px"><div align="center"><a href="#"><img id="botonBusqueda" src="../../img/eliminar.png" width="16" height="16" border="0" onClick="eliminar_equipo(<?php echo mysqli_result($res_resultado, $contador, "codbiometric")?>)" title="Eliminar"></a></div></td>
						</tr>
						<?php $contador++;
							}
						?>			
