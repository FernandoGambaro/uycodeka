...
class ZKLibrary {
	...
	public function getUser($varUser)
	{
		$command = CMD_USERTEMP_RRQ;
		...
		try
		{
			...
			if(count($this->user_data) > 0)
			{
				...
				while(strlen($user_data) > 72)
				{
					...
					$users[$uid] = array($userid, $name, $cardno , intval($role), $password);
					break;
					}elseif($varUser=='') {
					$cardno = hexdec( substr($u[1], 78, 2).substr($u[1], 76, 2).substr($u[1], 74, 2).substr($u[1], 72, 2) ).' '; 
					$role = hexdec(substr($u[1], 6, 2)).' ';            // 1 byte
	...
	public function getUserFace($uid, $face)
	{
		$template_data = '';
		$this->user_data = array();
		$command = CMD_USERFACE_RRQ;
		$byte1 = chr((int)($uid % 256));
		$byte2 = chr((int)($uid >> 8));
		$command_string = $byte1.$byte2.chr($finger);
		$chksum = 0;
	...
	public function setUserTemplate( $size, $pin, $finger_id,$valid,$template)
	{
		$uid = (int) $uid;
		$pin = (int) $pin;

		$command = CMD_USERTEMP_WRQ;
		$byte1 = chr((int)($pin % 256));
		$byte2 = chr((int)($pin >> 8));
				$template_size = strlen($template)+6;
		return $prefix = chr($template_size%256).chr(round($template_size/256)).$byte1.$byte2.chr($finge_idr).chr(1);
		$command_string = $prefix.$user_data;
