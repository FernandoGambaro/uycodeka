...
if ($cadena_busqueda<>"") {
	...
	$apellido=$array_cadena_busqueda[2];
	$usuario=$array_cadena_busqueda[3];
...
parent.document.getElementById("msganio").innerHTML="<i class='fa fa-long-arrow-right' aria-hidden='true'></i>&nbsp;  Gestión de usuarios ";
...
function OpenNote(noteId){
	$.colorbox({
	   	href: noteId, open:true,
			iframe:true, width:"750", height:"500",
...
function OpenList(noteId){
	$.colorbox({
		...
		function nuevo_usuario() {
			$.colorbox({href:"nuevo_usuarios.php",
			iframe:true, width:"750", height:"450",
		...
		function imprimir() {
			...
			if ($('#opcionesaexcel').is(":checked")){
				...
				if (opcionesimpresion==1) {
					url = "../excel/ListaDeUsuariosExcel.php?nombre="+nombre+"&apellido="+apellido+"&usuario="+usuario+"&telefono="+telefono;
				} 
					$.msgBox({ type: "prompt",
				...
				if (opcionesimpresion==1) {
					window.open("../fpdf/imprimir_usuarios.php?nombre="+nombre+"&apellido="+apellido+"&usuario="+usuario+"&telefono="+telefono);
				}
			}				
		}
		...
		function hacer_cadena_busqueda() {
			...
			var telefono=document.getElementById("telefono").value;
			var cadena="";
			cadena="~"+nombre+"~"+apellido+"~"+usuario+"~"+telefono+"~";
			return cadena;
			}
	...
	<body onLoad="inicio();" bgcolor="white">
		<div id="pagina">
			<div id="zonaContenido">
				...
				<table class="fuente8" width="100%"><tr><td>
					<table class="fuente8" cellspacing=0 cellpadding=3 border=0>					
						...
						if ($escribir=="true") { ?>						
						<button class="boletin" onClick="nuevo_usuario();" onMouseOver="style.cursor=cursor">
						<i class="fa fa-plus" aria-hidden="true"></i>&nbsp;Nuevo&nbsp;usuario</button>
						<?php } ?>							
						</td>
							<td>Nombre</td>
							<td><input id="nombre" name="nombre" type="text" class="cajaGrande" autocomplete="false" maxlength="45" value="<?php echo $nombre;?>"></td>
						</tr>
						<tr>
						<td>
						<?php 
						...
						<?php } ?>							
							...
							<td>Apellido</td>
							<td><input id="apellido" name="apellido" type="text" class="cajaGrande" autocomplete="false" maxlength="45" value="<?php echo $apellido;?>"></td>
						</tr>
			  	</table>
			  	</td><td>
			  	<table class="fuente8" cellspacing=0 cellpadding=3 border=0>
						<tr>
						  <td>Usuario</td>
						  <td><input id="usuario" type="text" class="cajaGrande" name="usuario" autocomplete="false" maxlength="45" value="<?php echo $usuario;?>"></td>
						...
						<tr>
						  ...
						  <td><input id="telefono" type="text" class="cajaPequena" name="telefono" autocomplete="false" maxlength="15" value="<?php echo $telefono;?>"></td>
					...
					<table class="fuente8" cellspacing=0 cellpadding=1 border=0>					    
						...
						<option value="1">Listado de usuarios</option>
			  ...
			  <table class="fuente8" width="90%" cellspacing=0 cellpadding=3 border=0 style="max-width: 800px;">
			  	<tr>
				<td width="30%" align="left">Nº encontrados <input id="filas" type="text" class="cajaMinima" name="filas" maxlength="5" readonly></td>
		...
		<td><input type="hidden" id="prevpagina" name="prevpagina" value="">
		...
		</td><td><input type="hidden" id="nextpagina" name="nextpagina" value="">
		...
		<td>Mostrados</td><td> <select name="paginas" id="paginas" onChange="paginar();" class="comboPequeno">
				...
				<input type="hidden" id="stylesel" name="stylesel">				
					...
					<iframe width="95%" height="340" id="frame_rejilla" name="frame_rejilla" frameborder="0" style="overflow-y: scroll; max-width: 800px;">
						<ilayer width="95%" height="340" id="frame_rejilla" name="frame_rejilla"></ilayer>
					</iframe>
